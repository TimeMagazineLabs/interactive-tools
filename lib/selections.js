...
let makeAutocompleteBox = function(selector, items, opts) {
	...
	function fireCallback(value) {
		let present = items.indexOf(value);

		if (present !== -1 && opts.onSelect) {
			opts.onSelect(value);
		}

		if (present === -1 && opts.onError) {
			opts.onError(value);
		}
	}
	...
	// if an option is clicked, blur the input afterward, which triggers the above
	...
	document.querySelector(selector).addEventListener("keydown", function(e) {
		if (e.keyCode === 9) {
			...
			let selectedCandidate = document.querySelector(selector + " li[aria-selected=true]");

			if (!selectedCandidate) {
				selectedCandidate = document.querySelector(selector + " li:hover");
			}
	...
	document.querySelector(selector).addEventListener("keyup", function(e) {
		if (e.keyCode === 13) {
			input.blur();
		}
	});
